...
Fit the simulated normal galaxy with BPASS SSPs.
'''
...
def main():
    ...
    with open('lgh_bpass.pkl','rb') as p:
        lgh_bp = pickle.load(p)

    lgh_bp.flux_obs = f['normal/BPASS/fnu'][:]
    lgh_bp.flux_unc = f['normal/BPASS/fnu_unc'][:]
    ...
    chain, logprob_chain, tau = lgh_bp.get_mcmc_chains(sampler,
                                                       discard=1000,
                                                       thin=50,
                                                       Nsamples=1000,
                                                       const_dim=const_dim,
    ...
    lnL = lgh_bp.get_model_likelihood(p0_lbfgs, negative=True)
    best = np.argmin(lnL)
    res = lgh_bp.fit(p0_lbfgs[best,:].flatten(),
                     method='optimize',
                     MCMC_followup=True,
                     force=True,
    ...
    chain2, logprob_chain2, tau2 = lgh_bp.get_mcmc_chains(res[1],
                                                          discard=500,
                                                          thin=25,
                                                          Nsamples=1000,
